class Producto:
    ...
    def __init__(self, nombre, precio, cantidad):
        try:
            ...
            if not isinstance(precio, (int, float)) or not self.check_non_negative(precio):
                raise ValueError('El precio no puede ser menor que 0')
            if not isinstance(cantidad, int) or not self.check_non_negative(cantidad):
    ...
    def __str__(self):
        return f"nombre: {self.nombre} - precio: {self.precio:2f} - cantidad: {self.cantidad}"

    def actualizar_precio(self, nuevo_precio):
        try:
            if not isinstance(nuevo_precio, (int, float)) or not self.check_non_negative(nuevo_precio):
                raise ValueError('El precio no puede ser menor que 0')
    ...
    def actualizar_cantidad(self, nueva_cantidad):
        try:
            if not isinstance(nueva_cantidad, int) or not self.check_non_negative(nueva_cantidad):
                raise ValueError('La cantidad no puede ser menor que 0')
...
def menu_principal(inv):
    while True:
        ...
        if opcion == '1':
            ...
            valor_total = inv.calcular_valor_inventario()
